"""Tool-use agent for customer-facing BMW sales chat."""
...
# Use Haiku for speed — tool calling + short responses don't need Sonnet
...
Your responses MUST be scannable in under 5 seconds:

1. **One-line lead** — direct answer or summary (max 15 words)
2. **Bullet points** — for multiple vehicles: `- **Vehicle Name** — ` + ONE short reason (max 12 words each)
3. **Optional closer** — one follow-up question (max 1 sentence)
...
CUSTOMER_TOOLS = [
    {
        ...
        "description": "Search the BMW vehicle inventory. Use this EVERY TIME the customer asks about vehicles, budget, preferences, or availability. Returns matching vehicles sorted by price.",
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Free-text search (model name, color, dealer, etc.)",
                },
                "series": {
                    "type": "string",
                    "description": "BMW series: 1, 2, 3, 4, 5, 7, 8, X1, X2, X3, X4, X5, X6, X7, Z4, i4, i5, i7, iX, iX1, iX2, iX3, M",
                },
                "fuel_type": {
                    "type": "string",
                ...
                "body_type": {
                    "type": "string",
                    "enum": ["SEDAN", "TOURING", "COUPE", "CONVERTIBLE", "SPORT_ACTIVITY_VEHICLE", "SPORT_ACTIVITY_COUPE", "GRAN_COUPE", "GRAN_TURISMO"],
                    "description": "Body type filter",
                },
                "drive_type": {
                    "type": "string",
                    "description": "Drive type filter, e.g. XDRIVE for AWD",
                },
                "color": {
                    "type": "string",
                    "description": "Color filter: WHITE, BLACK, GRAY, BLUE, RED, GREEN, SILVER, etc.",
                },
                "min_price": {
                    "type": "number",
    ...
    },
    {
        ...
        "description": "Get full details of a specific vehicle by VIN. Use when the customer asks about a specific car.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Compare two or more vehicles side by side. Use when the customer wants to decide between options.",
        "input_schema": {
            "type": "object",
            "properties": {
                "vins": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "List of VINs to compare",
                },
            },
            "required": ["vins"],
        },
    },
    {
        ...
        "description": "Get a summary of what's available: total count, price range, series breakdown, fuel types. Use for general 'what do you have?' questions.",
        "input_schema": {
            "type": "object",
            "properties": {},
        },
    },
    {
        ...
        "description": "Schedule a test drive for a specific vehicle. Use when the customer wants to test drive a car. Collects their name, email/phone, preferred date.",
        "input_schema": {
            "type": "object",
            "properties": {
                "vin": {"type": "string", "description": "VIN of the vehicle to test drive"},
                "customer_name": {"type": "string", "description": "Customer's name"},
                "contact": {"type": "string", "description": "Email or phone number"},
                "preferred_date": {"type": "string", "description": "Preferred date/time (e.g. 'next Saturday morning')"},
            },
            "required": ["vin"],
        },
    },
    {
        ...
        "description": "Book a service appointment for vehicle maintenance, repairs, tire changes, or inspections. Use when a customer needs their BMW serviced.",
        "input_schema": {
            "type": "object",
            "properties": {
                "service_type": {
                    "type": "string",
                    "enum": ["maintenance", "repair", "tire_change", "inspection", "recall", "other"],
                    "description": "Type of service needed",
                },
                "vehicle_description": {"type": "string", "description": "Customer's vehicle (model, year, or VIN if known)"},
                "customer_name": {"type": "string", "description": "Customer's name"},
                "contact": {"type": "string", "description": "Email or phone number"},
                "preferred_date": {"type": "string", "description": "Preferred date/time"},
                "description": {"type": "string", "description": "Description of the issue or service needed"},
            },
...
class CustomerAgentService:
    ...
    def _init_client(self):
        ...
        try:
            ...
            logger.error(f"Failed to init customer agent client: {e}")
    ...
    def chat(
        ...
        """Run customer agent with tool loop.

        Returns:
            {
                "message": str,          # The text response (with [RECOMMEND:] stripped)
                "recommended_vins": [],  # VINs extracted from [RECOMMEND:] tag
                "all_vehicle_vins": [],  # All VINs surfaced through tool calls
                "tool_calls": [],        # Tool call log for debugging
            }
        """
        if not self.client:
            return {
                "message": "I'm sorry, but the AI service is currently unavailable.",
                "recommended_vins": [],
                "all_vehicle_vins": [],
                "tool_calls": [],
            }
        ...
        tool_calls_log = []
        all_vehicle_vins = []  # Track all VINs surfaced through tools
        max_iterations = 6
        ...
        for _ in range(max_iterations):
            try:
                ...
                logger.error(f"Customer agent API error: {e}")
                return {
                    "message": "I apologize, but I encountered an error. Please try again.",
                    "recommended_vins": [],
                    "all_vehicle_vins": all_vehicle_vins,
            ...
            if response.stop_reason == "tool_use":
                ...
                return {
                    ...
                    "all_vehicle_vins": list(dict.fromkeys(all_vehicle_vins)),  # dedup, preserve order
                    "tool_calls": tool_calls_log,
                }

        return {
            "message": "I need a moment — could you rephrase your question?",
            "recommended_vins": [],
            "all_vehicle_vins": list(dict.fromkeys(all_vehicle_vins)),
            "tool_calls": tool_calls_log,
        }
    ...
    def chat_stream(
        ...
        Events:
            ...
            {"type": "vehicles", "vins": [...]}
            {"type": "done", "recommended_vins": [...], "all_vins": [...]}
        """
        if not self.client:
            yield {"type": "text_delta", "content": "I'm sorry, but the AI service is currently unavailable."}
        ...
        for _ in range(max_iterations):
            try:
                ...
                logger.error(f"Customer agent stream error: {e}")
            ...
            if response.stop_reason == "tool_use":
                ...
                # We already have the final response — extract text and stream it
                import re
                text = ""
                for block in response.content:
                    if hasattr(block, "text"):
                ...
                # Strip the tag and yield clean text
                clean_text = re.sub(r"\s*\[RECOMMEND:[^\]]*\]\s*", "", text).strip()
        ...
        yield {"type": "text_delta", "content": "I need a moment — could you rephrase your question?"}
    ...
    def _collect_vins(self, result: Any, vins: List[str]):
        """Extract VINs from tool results."""
    ...
    def _execute_tool(self, name: str, input: Dict[str, Any]) -> Any:
        try:
            ...
            logger.error(f"Customer tool {name} error: {e}")
            return {"error": str(e)}
    ...
    def _search_inventory(self, input: Dict) -> Dict:
        ...
        if input.get("drive_type"):
            term = f"%{input['drive_type']}%"
            query = query.filter(Vehicle.drive_type.ilike(term))
        if input.get("color"):
            term = f"%{input['color']}%"
            query = query.filter(Vehicle.color.ilike(term))
    ...
    def _get_vehicle_details(self, input: Dict) -> Dict:
        ...
        return {
            ...
            "color": v.color,
            "upholstery_color": v.upholstery_color,
            "price_chf": v.price_offer,
            "price_formatted": v.price,
            "monthly_installment": v.monthly_installment,
            "power_kw": v.power_kw,
    ...
    def _get_inventory_overview(self) -> Dict:
        ...
        return {
            ...
            "price_range_chf": {
                "cheapest": round(price_min) if price_min else None,
                "most_expensive": round(price_max) if price_max else None,
            },
        }
    ...
    def _schedule_test_drive(self, input: Dict) -> Dict:
        ...
        return {
            "status": "confirmed",
            "message": f"Test drive request registered for {vehicle_name}",
            "vehicle": vehicle_name,
            "dealer": dealer,
            "customer_name": input.get("customer_name", ""),
            "contact": input.get("contact", ""),
            "preferred_date": input.get("preferred_date", "to be confirmed"),
            "note": "The dealer will contact you within 24 hours to confirm the exact time.",
        }
    ...
    def _book_service_appointment(self, input: Dict) -> Dict:
        service_labels = {
            ...
            "inspection": "Vehicle Inspection (MFK)",
        ...
        try:
            ...
            self.db.add(ActivityLog(
                ...
                title=f"Service: {service_labels.get(service_type, service_type)}",
                description=f"{input.get('customer_name', 'Customer')} — {input.get('vehicle_description', '')}",
            ))
            self.db.commit()
        except Exception as e:
        ...
        return {
            "status": "confirmed",
            "message": f"Service appointment request registered: {service_labels.get(service_type, service_type)}",
            "service_type": service_labels.get(service_type, service_type),
            "vehicle": input.get("vehicle_description", ""),
            "customer_name": input.get("customer_name", ""),
            "contact": input.get("contact", ""),
            "preferred_date": input.get("preferred_date", "to be confirmed"),
            "description": input.get("description", ""),
            "note": "Your BMW Service Advisor will confirm the appointment and provide a cost estimate.",
        }
